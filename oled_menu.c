...
{
	float change2 = size + 5, change3 = 0; 
	int delay = 1;
	do
	{
		OLED_Clear();

		OLED_MoveCursor(0, &change2, delay + 3); 
		OLED_MoveCursor(128, &change3, delay);	 

		for (int i = 1; i <= arr_len; i++)
		{
			if (i == choose)
			{
				OLED_MoveCursor(i * size + 5, &now_y_arr[i], delay + 2); 
